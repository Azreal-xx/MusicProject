...
app.get('/api/getsearchresults',(req, res) => {
    request({
        ...
        const $ = cheerio.load(body)
        let datas = []
        $(".ibox-content").each((i, eles) => {
            let result = []
            let tags = ["手办"]
            let desc = ""
            let img = $(eles).children(".pull-left").children(".list-item-img").attr("src")
            let name = $(eles).children(".media-body").children(".media-heading").children("a").text()
            $(eles).children(".media-body").children("div").each((i, tag) => {
                
                $(tag).children("a").each((i, t) => {
                    let taget = $(t).children(".label-tag").text()
                    tags.push(taget)
                })
            })
            desc = $(eles).children(".media-body").children(".list-item-detail").text()
